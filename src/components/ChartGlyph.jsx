...
export function ChartGlyph({
  ...
  const up = "var(--color-secondary)";
  const down = "var(--color-destructive)";
  ...
  return (
    <svg
      ...
      {candles.map((cd, i) => {
        const color =
        sentiment === "Neutral" ? neutral : cd.up ? up : down;
        ...
        return (
          <g key={i} stroke={color} fill={color}>
            ...
            <rect x={cd.x - 4} y={top} width={8} height={bodyH} rx={1.5} opacity={0.95} />
          </g>);
