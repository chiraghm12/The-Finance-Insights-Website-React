...
export function InvertedHammer() {
  const slug = "inverted-hammer";
  ...
  const article = {
    ...
    readingTime: 7,
    sentiment: "Bullish",
    excerpt: "A bottom-of-trend candle hinting buyers are testing the waters."
  };
  ...
  const faqs = [
  {
    'q': 'Is the Inverted Hammer reliable for beginners?',
    'a': 'Yes — Inverted Hammer is beginner-friendly, but reliability improves dramatically when you combine it with trend context, volume, and strict risk management.'
  },
  {
    'q': 'Which timeframe works best for Inverted Hammer?',
  ...
  {
    'q': 'Do I need indicators to trade the Inverted Hammer?',
    ...
    <ArticleLayout article={article} related={related}>
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border bg-surface p-8 shadow-card">
        ...
        <div className="mt-6 flex w-full justify-center py-6 sm:py-8">
          <div className="aspect-[16/7] w-full">
            <ChartGlyph sentiment="Bullish" patternId="inverted-hammer" className="h-full w-full" />
          </div>
        </div>
      </div>
      ...
      <div className="mt-12 space-y-12">
        <Section icon={Lightbulb} title="Introduction">
          ...
          <p>Inverted Hammer is one of the most referenced concepts in candlestick patterns. In this guide we break it down visually, explain why it forms, and show you how disciplined traders turn it into a repeatable edge — without the jargon.</p>
        </Section>

        <Section icon={Layers} title="Pattern Structure">
          <p>The Inverted Hammer setup is defined by a clear, recognisable shape on the chart. We map out each component step by step, so you can spot a valid formation versus a look-alike that traps impatient traders.</p>
        </Section>

        <Section icon={Brain} title="Market Psychology">
          <p>Every Inverted Hammer is a snapshot of the battle between buyers and sellers. Understanding the emotional shift it captures — from euphoria to doubt, or fear to conviction — is what turns a shape into a signal you can trust.</p>
        </Section>

        <Section icon={Lightbulb} title="Formation Logic">
          <p>Inverted Hammer forms when a measurable imbalance between supply and demand resolves at a key level. Volume, prior trend, and location relative to support/resistance all determine whether the formation is high quality or low quality.</p>
        </Section>
        ...
        <Section icon={ListChecks} title="Identification Rules">
          <ul className="space-y-2.5">
              <li className="flex items-start gap-3">
                ...
                <span className="text-foreground">Confirm the prevailing trend before the Inverted Hammer appears.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-secondary/15 text-secondary">
                  <Check className="h-3 w-3" strokeWidth={3} />
        ...
        <Section icon={Target} title="Trading Strategy">
          <ol className="space-y-3">
              <li className="flex items-start gap-3 rounded-xl border border-border bg-card p-4">
                ...
                <span className="pt-0.5 text-foreground">Define your entry once the Inverted Hammer is confirmed — never anticipate it blindly.</span>
              </li>
              <li className="flex items-start gap-3 rounded-xl border border-border bg-card p-4">
                <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-gradient-accent text-sm font-bold text-secondary-foreground">
                  2
